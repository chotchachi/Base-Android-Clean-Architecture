...
class CatImageMapper : Mapper<CatImageEntity, CatImage> {
    override fun mapFromEntity(item: CatImageEntity) = CatImage(item.id, item.url, item.width, item.height)

    override fun mapToEntity(item: CatImage) = CatImageEntity(item.id, item.url, item.width, item.height)
}